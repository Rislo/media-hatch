...
export class ThreeMbLinksScraper {
    ...
    return HtmlMediaScraper.getHtmlDocument(scrapedMediaInfo.pageUrl).then(
      htmlDocument => {
        ...
        return this.getLinksPageHtml(linksPageUrl).then(htmlDocument => {
          ...
          scrapedMediaInfo.media.linksPackages = this.retrieveLinksPackages(
            htmlDocument.find("tr")
          );
  ...
  private getLinksPageHtml(linksPageUrl: string): Promise<Cheerio> {
    return WebRequest.post(
      "http://earn-money-onlines.info/wp-login.php?action=postpass",
      { jar: true, form: "post_password=300mbfilms&Submit=Submit" }
    ).then(response => {
      return HtmlMediaScraper.getHtmlDocument(linksPageUrl, {
  ...
  private linkRegex = xregexp.build(
    "(http://linkshrink.net.*)*(?<link>{{0}})",
    ["(http)(s)?://.*"],
    "i"
  );
