...
export class DdlValleyPostScraper {
  private tvShowsRegex = xregexp.build(
    "(?<title>{{0}})\\S*s(?<season>{{1}})(?:e(?<episode>{{2}}))*",
    ["\\S*", "[0-9]+", "[0-9]+"],
    "i"
  );
  ...
  public async scrape(htmlPost: Cheerio): Promise<HtmlScrapedMediaInfo[]> {
    ...
    if (isMovie) {
      ...
      if (movieTitleYearMatchArray && movieTitleYearMatchArray.length == 3) {
        movie.name = movieTitleYearMatchArray[1]
      ...
      if (tvShowMatchArray && tvShowMatchArray.length >= 3) {
        ...
        tvShow.name = tvShowMatchArray[1]
          .replace(new RegExp("\\.", "g"), " ")
          .trim();
        ...
        if (tvShowMatchArray.length > 3 && tvShowMatchArray[3]) {
          ...
          const linksPackages = await DdlValleyScraper.linksScraper.scrape(
            new HtmlScrapedMediaInfo(tvShow, pageUrl)
          );
          ...
          for (const linksPackage of linksPackages) {
            ...
            for (const link of linksPackage.links) {
              const linkTvShowMatchArray = xregexp.exec(
                link,
                this.tvShowsRegex
              );
              if (
                linkTvShowMatchArray &&
                linkTvShowMatchArray.length > 3 &&
                linkTvShowMatchArray[3]
              ) {
                const episodeNb = +linkTvShowMatchArray[3];
                if (!episodeNbToPackage.has(episodeNb)) {
                  episodeNbToPackage.set(
                    episodeNb,
                    new LinksPackage(linksPackage.fileHost, new Array<string>())
                  );
                }
            ...
            for (const episodeNb of episodeNbToPackage.keys()) {
              if (!episodeNbToTvShow.has(episodeNb)) {
                const tvShowClone = <TvShowEpisode>(
                  Object.assign(new TvShowEpisode(), tvShow)
                );
              ...
              episodeNbToTvShow
                .get(episodeNb)
                .linksPackages.push(episodeNbToPackage.get(episodeNb));
            }
          }
