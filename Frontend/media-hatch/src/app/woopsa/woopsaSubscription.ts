...
export class WoopsaSubscriptionChannel {
  ...
  constructor(private client: WoopsaClient, private size: number) {
    this.reset();
  }
  ...
  create(): Promise<number> {
    if (!this.channelCreationPromise) {
      ...
      this.channelCreationPromise = this.client
        .invoke(this.subscriptionCreate, { NotificationQueueSize: this.size })
  ...
  register(subscription: WoopsaSubscription): Promise<WoopsaSubscription> {
    ...
    return this.client.invoke(this.subscriptionRegister, args).then(subscriptionId => {
  ...
  unregister(subscription: WoopsaSubscription): Promise<void> {
    if (this.subscriptions.has(`${subscription.id}`)) {
      ...
      return this.client.invoke(this.subscriptionUnregister, args).then(success => {
  ...
  private wait() {
    ...
    this.client
      .invoke(this.subscriptionWait, args)
      .then(result => {
